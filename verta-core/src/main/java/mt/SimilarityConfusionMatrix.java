	...
	public double similarity(String[] featureNames, Word proposedWord, Word referenceWord) {
		...
		while (i < featureNames.length && hold) {
			...
			Double res = confusionMatrix
					.get(proposedWord.getFeature(featureName) + JOOIN_STRING + referenceWord.getFeature(featureName));
			if (res == null) {
				System.err.println("NOT found:" + proposedWord.getFeature(featureName) + JOOIN_STRING
						+ referenceWord.getFeature(featureName));
				w += defaultValue;
			} else {
				System.err.println("Found:" + proposedWord.getFeature(featureName) + JOOIN_STRING
