...
public class NgramMatchPro extends NgramMatch{
	...
	public SimilarityResult similarity(final Sentence s1, final Sentence s2, final SentenceAlignment dist,PrintStream strace) {
		...
		if(minsize>s1.size() || minsize>s2.size()) return SimilarityResult.bad;
		
		for(int s=Math.min(Math.min(minsize,s1.size()),s2.size());s<=Math.min(Math.min(maxsize,s1.size()),s2.size());++s) {		
			ngramprec += NgramMatch.compareNgrams(s,false,s1,s2,dist,strace) / MTsimilarity.totalngrams(s,s1.size());
		}
	   ...
	   strace.println("<t2s>");
		...
		ngramrec += NgramMatch.compareNgrams(s,true,s2,s1,dist,strace)/ MTsimilarity.totalngrams(s,s2.size());
		}
		...
		return new SimilarityResult(
				ngramprec==0 ? 0.0 : ngramprec / (maxsize1-minsize1 +1 ) , 
