...
public class SimilarityResult {
	...
	SimilarityResult(double prec, double rec) {
		this.rec=rec;
		this.prec=prec;
	}
	
	SimilarityResult(int res0, int size0,  int size1) {
	
	if(size0==0 && size1==0) { prec=1; rec=1;}
	else {
	...
	public double getRec(){return rec;}
	public double getF1() { return prec>0 && rec> 0  ? (( 2 * getPrec() * getRec() ) /   (getPrec() + getRec())) : 0;}
